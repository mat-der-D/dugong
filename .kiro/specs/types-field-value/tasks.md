...
- [x] 1. フィールド値の統一インターフェース（FieldValue）の定義と全テンソル型への実装
- [x] 1.1 FieldValue の共通インターフェースを定義する
...
- [x] 1.2 すべてのテンソル型に FieldValue を実装する
...
- [x] 2. 微分演算子の出力型をコンパイル時に決定するランク昇降 trait の定義
- [x] 2.1 (P) 勾配演算子の出力型を決定する HasGrad trait を定義し実装する
...
- [x] 2.2 (P) 発散演算子の出力型を決定する HasDiv trait を定義し実装する
...
- [x] 4.2 (P) HasGrad / HasDiv の型レベルテストと compile_fail テストを追加する
